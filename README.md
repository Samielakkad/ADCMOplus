...
*verifiable without MATLAB* a compact Python reference implementation
reproduces every component. The figures below are generated by it.

**It tracks a moving, constrained Pareto front.** As objectives drift and the
forbidden region (grey gap) sweeps across the front, the population continuously
re-converges:

<div align="center">
<img src="figures/pareto_tracking.gif" width="460" alt="ADCMOplus tracking a moving constrained Pareto front">
</div>

**It recovers faster than naive restart — ~39% lower mean IGD.** After every
change the prediction-based response snaps back below a random-restart baseline:

![IGD over time](figures/igd_over_time.png)

**The classifier recovers the change type, and the regret bound holds.** The
magnitude-ratio classifier identifies whether the objectives, the constraints,
or both changed (left); and the Hedge meta-policy's realised regret stays under
the proven `√((T/2)·ln K)` bound at every horizon (right):

<p align="center">
<img src="figures/classifier_confusion.png" width="360">
<img src="figures/regret_bound.png" width="430">
</p>

> See [`METHOD.md`](METHOD.md) for the full algorithm and the math behind each
> component.
...
reach out — happy to share for academic or technical review.
